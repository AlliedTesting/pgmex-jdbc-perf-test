...
%       fileNameList: cell[1,nFigures] of char[1,] - list of resulting file
%           names, if not specified file names are generated based on
%           figure names
%
...
import mxberry.core.genfilename;
...
%
tmpDir=mxberry.io.TmpDataManager.getDirByCallerKey();
%
    ...
    if ~ishandle(hFigure)
        ...
        try
            ...
            for iFormat=1:numel(formatNameList)
                saveInFormat(fileNameList{iFigure},...
                    formatNameList{iFormat});
            end
...
mxberry.io.rmdir(tmpDir,'s');
    function saveInFormat(shortFigFileName,formatName)
        ...
        logger.debug([msgStr,'...']);
        %
        tmpFileName=[tmpDir,filesep,mxberry.core.hash(figFileName),'.',...
            formatName];
        %
        ...
        mxberry.io.copyfile(tmpFileName,figFileName);
        %
