classdef ATestCompareWithJDBC < matlab.unittest.TestCase
    ...
    methods (TestClassSetup)
        function self = set_up(self)
            ...
            if strcmpi(testModeName,'jdbc')
                self.dbExecFunc=self.MODE_MAP_CMAT{find(strcmp(...
                    self.MODE_MAP_CMAT(:,1),'jdbc'),1),2};
            else
                self.dbExecFunc=self.MODE_MAP_CMAT{find(strcmp(...
                    self.MODE_MAP_CMAT(:,1),'pgmex'),1),2};
            end
            %
    ...
    methods (Access=protected)
        function setUpTestMode(self,testModeName)
            switch lower(testModeName)
                case 'pgmex'
                    self.filterFuncNameCVec={'batchParamExec'};
                case 'jdbc'
                    self.filterFuncNameCVec={'datainsert','fastinsert'};
                case 'all'
                    self.filterFuncNameCVec=...
    ...
    methods (Access=private)
        function funcNameCVec=filterFuncNameList(self,...
                funcNameCVec)
        ...
        function self=compareInsertInternal(self,...
            ...
            STestData=struct(...
                ...
                'timeMat',{nanmedian(timeMat,3)},...
                'prepareTimeMat',{nanmedian(prepareInsertTimeMat,3)},...
                'selfTimeMat',{nanmedian(selfInsertTimeMat,3)},...
                'prepareDBTimeVec',{nanmedian(reshape(...
                prepareTimeMat,nSamples,[]),2)},...
    ...
    methods (Static)
        function hFigVec=plot(varargin)
            [reg,~,filterFuncNameCVec,selfTimeMode,...
                xLimVec,yLimVec,legendLocation,...
    ...
    methods (Access=private,Static,Hidden)
        function hFigure=plotTrajInternal(figName,...
                ...
                javaMaxMemory,selfTimeMode,xLimVec,yLimVec,legendLocation)
            dataSizeOutFunc=...
        ...
        function hFigure=plotBarInternal(figName,...
            ...
            for iPlot=1:nPlotsPerFunc
                timeVec=nanmean(scaledTimeMat(:,:,iPlot),1);
